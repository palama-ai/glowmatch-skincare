...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    // حفظ تفاصيل الخطأ في الحالة لعرضها أثناء التطوير
          ...
          <div className="text-center p-8 max-w-md">
            ...
            <div className="flex flex-col gap-1 text-center">
              ...
              {process.env.NODE_ENV !== 'production' && this.state?.error && (
                <details className="mt-4 text-left p-3 bg-white border rounded text-sm text-red-700">
                  <summary className="font-medium">Error details (click to expand)</summary>
                  <pre className="whitespace-pre-wrap mt-2 text-xs">{this.state.error?.message}</pre>
                  <pre className="whitespace-pre-wrap mt-2 text-xs">{this.state.errorInfo?.componentStack}</pre>
                </details>
              )}
            </div>
